...
def create_gdf(shapefile_paths, beltline_geom):
    """ Creates a new combined Geodataframe; modify each layer's geodataframe, then combine into one Combined Geodataframe """
    gdfs = []
    num_geometries = []
...
def standardize_gdf(gdf):
    # Sort by ID
    gdf = gdf.sort_values(by="Simulation_ID", ascending=True)
    # Rebuild geometries in a canonical orientation so unary_union is consistent
    gdf["geometry"] = gdf["geometry"].map(lambda geom: shapely.ops.orient(geom, sign=1.0))
    # Return new object or in-place
