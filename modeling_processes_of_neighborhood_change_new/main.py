...
from helper import create_required_directories, GDF_CACHE_FILENAME, GIFS_CACHE_DIR, PLT_DIR, T_MAX_L, SAVED_IDS_FILE, SAVED_BLMETERS_FILE, BLMETERS_LIST, SAVED_LAYER_URLS_FILE, LAYER_CACHE_DIR, ZIP_URLS
...
# Suppress warning about OSMnx graph generation querying too large of an area.
import warnings
warnings.filterwarnings(
    "ignore",
    message=r"This area is .* your configured Overpass max query area size.*",
...
def main():
    # ===============================================
    # Create all directories after potential deletion
    # ===============================================
    create_required_directories()
    
    # ==================================================
    # CHECK IF REGIONS, BLMETERS, OR LAYERS HAVE CHANGED
    # ==================================================
    regen_gdf = False # Toggles True if: 1) Regions to simulate have changed; 2) Beltline score radius has changed; 3) Layers have changed
    
    saved_IDS = load_previous_values(SAVED_IDS_FILE)
    saved_BLMETERS = load_previous_values(SAVED_BLMETERS_FILE)
    saved_LAYER_URLS = load_previous_values(SAVED_LAYER_URLS_FILE)
    
    if set(saved_LAYER_URLS) != set(ZIP_URLS):
        # Helper function to delete 'cache/layers'
        ...
        # Delete 'cache/layers' folder to clear it
        if LAYER_CACHE_DIR.exists():
            shutil.rmtree(LAYER_CACHE_DIR, ignore_errors=False, onerror=_handleRemoveReadonly)
        regen_gdf = True
        
        os.makedirs(LAYER_CACHE_DIR, exist_ok=True)
        
        create_required_directories()
    elif set(saved_IDS) != set(ID_LIST) or saved_BLMETERS != BLMETERS_LIST or not (GDF_CACHE_FILENAME).exists():
        regen_gdf = True
    
    save_current_values(ID_LIST, SAVED_IDS_FILE)
    save_current_values(BLMETERS_LIST, SAVED_BLMETERS_FILE)
    save_current_values(ZIP_URLS, SAVED_LAYER_URLS_FILE)
    ...
    print(f"[ECONOMIC DISTRIBUTION]: Used {n} census tracts (2010 Fulton+Dekalb) to calculate.\n")
    ...
    if regen_gdf or viewData:
        print("Initializing 'Beltline' geometry...")
    ...
    # Create or load GDF
    if regen_gdf:
        gdf, num_geometries, num_geometries_individual = create_gdf(shapefile_paths, beltline_geom)
    else:
...
# TO-DO LIST
""" High Priority """
#TODO: Acquire optimal set of parameters to match 2010 data [Investigate ML approach]
#TODO: For 2010 calibration, query amenities from 2010 [Matthew]
#TODO: Address disconnected census tracts from Graph 
    #- Investigate "not strongly connected graph"
#TODO: Add weights to amenity types [Easy]
#TODO: Investigate funky amenity counts

""" Medium Priority """
#TODO: Since data from multiple layers/graphs are cached in the same folders; verify that the proper data is being used for each (check data CSV's)
    # - Centroid distances, amenities [Matthew]
#TODO: add epsilon as a changeable parameter (check functionality in simulation)
#TODO: open economic/population links before attempting download [please do this]

""" Low Priority """
#TODO: Change centroid distance to be avg of: shortest paths between every node in a region A to every node in region B
#TODO: Keep all layers cached, function without having to redownload [use hasher.py]
#TODO: Add region-specific tag to image file names
#TODO: Make amenity queries faster

#TODO: Keep all combined_gdf's cached [use hasher.py]
#TODO: Check if gif already exists before re-creating it
#TODO: In calibration CSV's, order parameters from least to greatest
